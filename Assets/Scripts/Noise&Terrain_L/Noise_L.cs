...
public static class Noise
{
    ...
    public static float[,] GenerateNoiseMap_L(int mapWidth, int mapHeight, int seed, float scale, int octaves, float persistance, float lacunarity, Vector2 offset, NormalizeMode normalizeMode)
    {
        ...
        // if scale becomes 0, will get 'Div by 0' error, so we clamp it at a really small value.
        if (scale <= 0)
        {
            scale = 0.0001f;
        }
